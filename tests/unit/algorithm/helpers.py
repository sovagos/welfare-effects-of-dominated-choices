...
from python.algorithm.applicant import Applicant, InitialApplicantStatus, AdmittedApplicantStatus, \
    RejectedApplicantStatus, Application
...
def create_applicant(override=None) -> Applicant:
    if not override:
        override = {}
    return Applicant(**{"ranked_applications": [], "status": InitialApplicantStatus(), **override})


def create_application(override=None) -> Application:
    if not override:
        override = {}
    return Application(**{"contract": get_random_string(), "priority_score": get_random_float(), **override})
