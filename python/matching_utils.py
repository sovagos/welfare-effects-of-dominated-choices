...
def summarize_dominated_choices(applicants):
    ...
    for applicant in applicants.values():
        ...
        dominated_choice += int(
            any([applicant.dominated_dropping, applicant.dominated_flipping])
        )
    print(
        f"Share of applicants with dominated dropping: {dominated_dropping/len(applicants)}"
    )
    print(
        f"Share of applicants with dominated flipping: {dominated_flipping/len(applicants)}"
    )
    print(
        f"Share of applicants with dominated choices: {dominated_choice/len(applicants)}"
    )
...
def add_single_tie_breaker(applicants, seed=1000):
    ...
    for applicant in applicants.values():
        applicant.priority_scores_sorted = [
            priority_score + random_tiebreaker[applicant.applicant_id]
            for priority_score in applicant.priority_scores_sorted
        ]
...
def student_proposing_deferred_acceptance(applicants, contracts):
    ...
    while len(proposers) > 0:
        ...
        # Rejected applicant
        while (
            contracts[contract_id].score_dictionary[applicant_id] < cutoffs[contract_id]
        ) | (
            contracts[contract_id].score_dictionary[applicant_id]
            < float(PRIORITY_SCORE_CUTOFF_MIN)
        ):
        ...
        # Update assignment
        heappush(
            temporary_allocation[contract_id],
            (contracts[contract_id].score_dictionary[applicant_id], applicant_id),
        )
        if len(temporary_allocation[contract_id]) > contracts[contract_id].capacity:
            ...
            if current_index[applicant_id_out] < len(
                applicants[applicant_id_out].ranking_sorted
            ):
